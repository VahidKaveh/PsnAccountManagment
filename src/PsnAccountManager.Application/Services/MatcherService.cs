...
public class MatcherService : IMatcherService
{
    ...
    {
        _logger.LogInformation("Matcher service started for a request with {GameCount} games.",
            request.RequestedGameIds.Count);
        ...
        var sortOrder =
            await _settingRepository.GetValueAsync("Matcher.SuggestionSortOrder", SuggestionSortOrder.ByMatchedGames);
        ...
        {
            _logger.LogInformation("No primary match found that meets the minimum requirement of {MinGames} games.",
                minMatchedGames);
            return result;
        }

        result.Suggestions.Add(MapToMatchedAccountDto(bestMatchAccount, bestMatchGameIds, true));
        _logger.LogDebug("Primary match found: AccountId {AccountId} with {MatchCount} games.", bestMatchAccount.Id,
            bestMatchGameIds.Count);
        ...
        foreach (var account in otherAccounts)
        {
            ...
            if (matchedIds.Any()) secondaryMatches.Add(MapToMatchedAccountDto(account, matchedIds, false));
        }
        ...
        _logger.LogInformation("Matcher service finished. Found {TotalSuggestions} suggestions.",
    ...
    {
        var displayPrice = account.PricePs5 ?? account.PricePs4 ?? 0;
